...
- `press_key`/`fill_input` can DOUBLE every character on newer Chrome (confirmed HeadlessChrome 149: typing `"abc"` yields `"aabbcc"`, `m3media` → `mm33mmeeddiiaa`). Cause: `press_key` dispatches `keyDown` *with* a `text` field AND a separate `char` event — newer Chrome already inserts the character on `keyDown(text)`, so the `char` event duplicates it. Symptom looks like a wrong value or "invalid username/password" on login forms. Workaround for React/Vue/controlled inputs: skip the keyboard entirely and use the native value setter + events — `Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(el, v); el.dispatchEvent(new Event('input',{bubbles:true})); el.dispatchEvent(new Event('change',{bubbles:true}))`. Real fix: `press_key` should not put `text` on the `keyDown` (let the `char` event do the single insert), or send `rawKeyDown`+`char`.
