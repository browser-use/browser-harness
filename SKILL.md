...
- **On macOS, use `open -a "Google Chrome" <url>`, never AppleScript `tell application`.** Apple Events are gated behind a macOS Automation consent dialog — the square "wants access to control Google Chrome" popup. The grant is keyed to the calling binary's exact path, and Homebrew's node is `/opt/homebrew/Cellar/node/<version>/bin/node`, so every node upgrade invalidates it and the popup returns forever. `open` needs no grant. (It may route through the profile picker if Chrome is not already running; that is the cheaper problem.)
